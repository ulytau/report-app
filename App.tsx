          ...
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex space-x-8">
              <button 
                ...
                className={`py-3 px-1 border-b-2 font-medium text-sm transition-all ${activeTab === 'overview' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
              >
                Общий обзор
              </button>
              <button 
                onClick={() => setActiveTab('hourly')}
                className={`py-3 px-1 border-b-2 font-medium text-sm transition-all ${activeTab === 'hourly' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
              >
                Анализ по часам
              </button>
            </div>
          ...
          <div ref={reportRef} id="hourly-report" className="space-y-8 animate-in slide-in-from-right duration-500 p-4 -m-4">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-xl font-bold text-slate-800">Почасовая активность (24 часа)</h3>
                  <p className="text-slate-500 text-sm">Распределение выручки по времени суток</p>
                </div>
              ...
              <div className="mt-8 grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 gap-2">
                {data.revenueByHour.map((h, i) => (
                  <div key={i} className="flex flex-col items-center p-2 rounded-xl bg-slate-50 border border-slate-100">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">{h.hour}</span>
